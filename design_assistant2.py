...
class DesignAssistant:
    ...
    def process_input(self, text_prompt=None, image_path=None):
        ...
        css = self.css_code[best_idx]
        js = self.js_code[best_idx]
        cluster_label = int(self.labels[best_idx])

        return {'css_code': css, 'js_code': js, 'cluster_label': cluster_label}

    def save_output_files(self, css_code, js_code, output_dir='output', base_filename='generated_code'):
        os.makedirs(output_dir, exist_ok=True)

        css_path = os.path.join(output_dir, f'{base_filename}.css')
        js_path = os.path.join(output_dir, f'{base_filename}.js')

        # Save CSS with pretty formatting (you can enhance formatting if needed)
        with open(css_path, 'w', encoding='utf-8') as f_css:
            f_css.write(css_code.strip() + '\n')

        # Save JS with pretty formatting (you can enhance formatting if needed)
        with open(js_path, 'w', encoding='utf-8') as f_js:
            f_js.write(js_code.strip() + '\n')

        print(f"CSS code saved to: {css_path}")
        print(f"JavaScript code saved to: {js_path}")
...
if __name__ == "__main__":
    ...
    # Example text prompt
    prompt = "create a fashion website with fully responsive format"
    ...
    assistant.save_output_files(result['css_code'], result['js_code'], base_filename='output_from_text_prompt')
    ...
    # assistant.save_output_files(result_img['css_code'], result_img['js_code'], base_filename='output_from_image_input')
